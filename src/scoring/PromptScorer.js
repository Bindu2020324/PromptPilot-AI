...
function scoreClarity(text) {
  ...
  if (wordCount < 3)
    return {
      score: 5,
      tips: ['Your prompt is too short to be clear. Add more detail.'],
    };
  ...
  if (
    text.includes('?') ||
    /^(please |kindly )?(implement|design|create|build|write|develop|explain)/i.test(
      text.trim()
    )
  ) {
    score += 10;
...
function scoreSpecificity(text) {
  ...
  if (wordCount < 3)
    return {
      score: 5,
      tips: ['Add specific details, technologies, and requirements.'],
    };
  ...
  if (numbers.length >= 3) score += 15;
    ...
    tips.push(
      'Add concrete numbers (e.g., "5 items", "3 sections", "under 200ms")'
    );
...
function scoreActionability(text) {
  ...
  if (wordCount < 3)
    return {
      score: 5,
      tips: ['Use action verbs to make your prompt actionable.'],
    };
  ...
  if (actionCount >= 5) score += 30;
    ...
    tips.push(
      'Start with action verbs: implement, design, analyze, evaluate...'
    );

  // Imperative mood (starts with verb)
  if (
    /^(implement|design|create|build|develop|write|generate|analyze|explain|list|define|compare)/i.test(
      text.trim()
    )
  ) {
    score += 15;
  ...
  if (
    /deliver|output|result|return|provide|produce|generate|create/i.test(text)
  ) {
    score += 10;
  } else {
  ...
  const tasks =
    text.split(/\band\b|\bthen\b|\balso\b|\badditionally\b/i).length - 1;
  if (tasks >= 2) score += 10;
...
function scoreContext(text) {
  ...
  if (wordCount < 5)
    return { score: 5, tips: ['Add background context to your prompt.'] };
  ...
  if (contextCount >= 5) score += 30;
    ...
    tips.push(
      'Add context: "Given that...", "In the context of...", "The goal is..."'
    );

  // Role or persona mentioned
  if (
    /as a|you are|act as|role|persona|expert|senior|junior|beginner/i.test(text)
  ) {
    score += 15;
  } else {
    tips.push(
      'Assign a role: "As a senior developer..." or "You are an expert in..."'
    );
  }

  // Audience mentioned
  if (
    /audience|user|reader|developer|beginner|student|customer|client|team/i.test(
      text
    )
  ) {
    score += 10;
  ...
  if (
    /currently|existing|we have|our|project|system|application|codebase/i.test(
      text
    )
  ) {
    score += 10;
...
function scoreConstraints(text) {
  ...
  if (words.length < 5)
    return {
      score: 5,
      tips: ['Add constraints and limitations to your prompt.'],
    };
  ...
  if (constraintCount >= 5) score += 35;
    ...
    tips.push(
      'Add constraints: "must", "should not", "limited to", "within X"'
    );
  ...
  if (
    /deadline|time|quick|brief|concise|short|long|comprehensive|thorough/i.test(
      text
    )
  ) {
    score += 10;
...
function scoreOutputFormat(text) {
  const words = text.split(/\s+/).filter((w) => w.length > 0);
  if (words.length < 3)
    return { score: 5, tips: ['Specify the desired output format.'] };
  ...
  if (formatCount >= 4) score += 35;
    ...
    tips.push(
      'Specify output format: "Return as JSON", "Use a table", "Step-by-step list"'
    );
  ...
  if (
    /\b(brief|detailed|comprehensive|paragraph|sentence|word|line|page)\b/i.test(
      text
    )
  ) {
    score += 10;
...
function scoreEdgeCases(text) {
  ...
  if (words.length < 5)
    return {
      score: 5,
      tips: ['Consider mentioning edge cases and error handling.'],
    };
  ...
  if (edgeCount >= 4) score += 40;
    ...
    tips.push(
      'Mention edge cases: "Handle empty inputs", "What if the API fails?"'
    );
...
function scoreBestPractices(text) {
  ...
  if (words.length < 5)
    return {
      score: 5,
      tips: ['Reference industry best practices and standards.'],
    };
  ...
  if (bpCount >= 5) score += 35;
    ...
    tips.push(
      'Reference standards: "Follow SOLID principles", "Use TypeScript", "Include tests"'
    );

  // Quality attributes
  if (
    /quality|reliable|robust|secure|efficient|performant|scalable|maintainable/i.test(
      text
    )
  ) {
    score += 15;
...
export function scorePrompt(text) {
  if (!text || typeof text !== 'string') {
    return {
      dimensions: Object.fromEntries(
        DIMENSIONS.map((d) => [d.id, { score: 0, tips: [] }])
      ),
      overall: 0,
  ...
  const sortedTips = allTips.sort((a, b) => {
    const aScore =
      dimensions[DIMENSIONS.find((d) => d.label === a.dimension)?.id]?.score ||
      0;
    const bScore =
      dimensions[DIMENSIONS.find((d) => d.label === b.dimension)?.id]?.score ||
      0;
...
export function compareScores(originalText, enhancedText) {
  ...
  const improvements = DIMENSIONS.map((dim) => ({
    ...
    delta:
      (enhanced.dimensions[dim.id]?.score || 0) -
      (original.dimensions[dim.id]?.score || 0),
  }));
